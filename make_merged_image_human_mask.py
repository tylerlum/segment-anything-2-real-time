...
# Human masks directory (optional - for keeping human visible in merged images)
HUMAN_MASK_DIR = DEMO_DIR / "human_masks"
HAS_HUMAN_MASKS = HUMAN_MASK_DIR.exists()
if HAS_HUMAN_MASKS:
    print(f"Found human masks directory: {HUMAN_MASK_DIR}")
else:
    print(f"No human masks directory found at {HUMAN_MASK_DIR} - human regions will be averaged")
...
# Load human masks if available
if HAS_HUMAN_MASKS:
    HUMAN_MASK_FILES = sorted(list(HUMAN_MASK_DIR.glob("*.png")))
    if len(HUMAN_MASK_FILES) > 0:
        human_mask_images = [Image.open(HUMAN_MASK_FILES[i]) for i in FRAME_INDICES]
        print(f"Loaded {len(human_mask_images)} human mask images")
    else:
        human_mask_images = None
        print("No human mask files found")
else:
    human_mask_images = None
...
def create_pose_sequence_figure(
    ...
    mask_images: list,
    pose_colors: list,
    human_mask_images: list = None,
    background_alpha: float = 0.25,
    human_alpha: float = 1.0,  # 1.0 = full opacity for correct colors
    ...
    Args:
        human_mask_images: Optional list of human mask images. If provided, human regions
                          will be composited using MAX (not average) to stay visible.
        background_alpha: Alpha for non-human background regions (lower = more faded).
        human_alpha: Alpha for human regions (higher = more visible).
    ...
    if use_first_frame_only:
        ...
        # Create composite background from all frames
        # For non-human regions: average (will be faded)
        # For human regions: paint each frame's human on top (last frame wins, full opacity)
        ...
        combined_human_mask = None
        if human_mask_images is not None:
            combined_human_mask = np.zeros((H, W), dtype=bool)
            for human_mask_img in human_mask_images:
                combined_human_mask |= get_mask_bool(human_mask_img)
    
    # Apply fading to background (non-human regions)
    ...
    # Now paint human regions from each frame at FULL opacity (no fading)
    # This ensures humans look solid and have correct colors
    if human_mask_images is not None and not use_first_frame_only:
        for rgb_img, human_mask_img in zip(rgb_images, human_mask_images):
            rgb_array = np.array(rgb_img).astype(np.float32)
            human_mask = get_mask_bool(human_mask_img)
            
            # Paint human at full opacity (or slightly faded with human_alpha)
            human_rgb = rgb_array * human_alpha + 255 * (1 - human_alpha)
            
            for c in range(3):
                composite[:, :, c] = np.where(human_mask, human_rgb[:, :, c], composite[:, :, c])
...
pose_sequence_img = create_pose_sequence_figure(
    ...
    human_mask_images=human_mask_images,
    background_alpha=0.3,  # Non-human background faded
    human_alpha=0.4,  # Human semi-transparent (0.6 = 60% visible, 40% faded to white)
)
...
def create_single_pose_with_context(
    ...
    Args:
        human_mask_images: Optional list of human mask images for keeping human visible.
        background_alpha: Alpha for non-human background regions.
        human_alpha: Alpha for human regions (higher = more visible).
    ...
    if use_first_frame_only:
        ...
        # Average all frames for background
        bg_composite = np.zeros((H, W, 3), dtype=np.float32)
        for rgb_img in rgb_images:
            bg_composite += np.array(rgb_img).astype(np.float32)
        bg_composite /= len(rgb_images)
    
    # Apply fading to background
    if use_grayscale_background:
        bg_gray = np.mean(bg_composite, axis=2, keepdims=True)
        bg_faded = bg_gray * background_alpha + 255 * (1 - background_alpha)
        bg_faded = np.repeat(bg_faded, 3, axis=2)
    ...
    # Paint human regions from each frame at full opacity (correct colors, no ghosting)
    if human_mask_images is not None and not use_first_frame_only:
        for rgb_img, human_mask_img in zip(rgb_images, human_mask_images):
            rgb_array = np.array(rgb_img).astype(np.float32)
            human_mask = get_mask_bool(human_mask_img)
            
            # Paint human at specified alpha (1.0 = full opacity)
            human_rgb = rgb_array * human_alpha + 255 * (1 - human_alpha)
...
for i, (color, name) in enumerate(zip(POSE_COLORS, POSE_NAMES)):
    single_pose_img = create_single_pose_with_context(
        ...
        human_alpha=0.4,  # Human semi-transparent
